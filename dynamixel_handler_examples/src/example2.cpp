...
int main() {
    ...
    auto dxl = dynamixel_handler::msg::DxlStates();
    auto updated_time = std::map<std::string, rclcpp::Time>();

    auto node  = std::make_shared<rclcpp::Node>("example1_node");
    auto pub_cmd = node->create_publisher<dynamixel_handler::msg::DxlCommandsX>("dynamixel/commands/x", 10);
    auto sub_st = node->create_subscription<dynamixel_handler::msg::DxlStates>("dynamixel/states", 10, 
        [&](const dynamixel_handler::msg::DxlStates::SharedPtr msg){
    ...
    auto timer = node->create_wall_timer(1s, [&](){
        ...
        auto cmd = dynamixel_handler::msg::DxlCommandsX();
    });
